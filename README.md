...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly create MajorsBetting-f994
  fly create MajorsBetting-f994-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app MajorsBetting-f994
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app MajorsBetting-f994-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'MajorsBetting-f994-staging' does not match app name in config file 'MajorsBetting-f994'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name MajorsBetting-f994-db
  fly postgres attach --postgres-app MajorsBetting-f994-db --app MajorsBetting-f994

  fly postgres create --name MajorsBetting-f994-staging-db
  fly postgres attach --postgres-app MajorsBetting-f994-staging-db --app MajorsBetting-f994-staging
  ```
